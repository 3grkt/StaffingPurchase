(function () {
    ...
    angular
        .module('spApp.report', ['spApp.report.service'])
        .controller('internalResitionFormCtrl', ['$uibModal','$translate','reportService','productService', 'userService', 'Page', 'Notification', 'commonService', 'gridService', internalResitionFormCtrl]);

    function internalResitionFormCtrl($uibModal, $translate, reportService, productService, userService, Page, Notification, commonService, gridService) {
        var self = this;

        Page.setTitle('PageTitle.AdminReport.InternalRequisitionForm');
        ...
        self.gridOrderReport.columnDefs = [
            ...
            { field: 'SKUName', displayName: 'SKU Name', headerCellFilter: 'translate' },
            { field: 'Quantity', displayName: 'Quantity', headerCellFilter: 'translate' },
            { field: 'UnitPrice', displayName: 'Unit Price (VND)', headerCellFilter: 'translate', cellFilter: 'currency:"":0' },
            { field: 'TotalPrice', displayName: 'Total Price (VND)', headerCellFilter: 'translate', cellFilter: 'currency:"":0' },
            { field: 'PV', displayName: 'PV/ea', headerCellFilter: 'translate' },
            { field: 'TotalPV', displayName: 'Total PV', headerCellFilter: 'translate' },
        ...
        self.search = function () {
            if (self.selectedBatchDate) {
                ...
                reportService.searchInternalRequisitionForm(filter, self.paginationOptions)
                    .then(function (data) {
        ...
        self.bindSearchResult = function(data) {
            ...
            self.summaryTotalPV = data.SummaryTotalPV;
        ...
        self.export = function() {
            if(self.selectedBatchDate){
                ...
                reportService.downloadInternalRequisitionForm(filter).then(function(data){
